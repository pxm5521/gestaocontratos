...
**Sobre o tamanho dos contratos**: contratos de até ~110 mil caracteres (a grande maioria — algo como 40-50 páginas densas) são analisados **por inteiro**, sem nenhum corte. Para documentos maiores que isso, o site mostra quantas páginas o PDF tem e pede duas informações específicas — **em quais páginas está a tabela de preços** (código do item, preço unitário e quantidade contratada, quando houver) e **em quais páginas estão os marcos de faturamento** — além de um campo opcional para outras páginas gerais (dados das partes, cláusulas). Indicar essas páginas melhora bastante a precisão da extração. Se deixar tudo em branco, o site tenta localizar sozinho as seções mais relevantes.
...
Além do modelo de BM, agora dá para cadastrar também um **modelo de Proforma** (mesma lógica de tokens). Os modelos usam os tokens `{{LINHA1_...}}`, `{{LINHA2_...}}` etc. (descrição, unidade, quantidade, valor unitário, código) para cada item incluído no marco. Se o número de itens for maior do que o modelo já tem linhas prontas, **o sistema insere automaticamente as linhas que faltarem**, copiando o formato da última linha e ajustando as fórmulas de soma — não tem limite fixo de itens por documento. Slots que sobrarem sem item (quando há menos itens do que linhas prontas no modelo) ficam em branco, zerando o total daquela linha sem precisar apagar nada na planilha.
